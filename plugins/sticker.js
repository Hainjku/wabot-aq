    ...
    if ((isMedia && !m.message.videoMessage || isQuotedImage) && args.length == 0) {
      ...
      await ffmpeg(`./${media}`)
        ...
        .on('error', function (e) {
          ...
          m.reply(`\`\`\`Falló, en el momento de la conversión ${tipe} el sticker debe durar menos de 10 segundos\`\`\``)
        })
        .on('end', function () {
          console.log('Finish')
          buff = fs.readFileSync(ran)
      ...
      await removeBackgroundFromImageFile({ path: media, apiKey: "ku7CybpBNXacsoWMyeZeLGQq", size: 'auto', type: 'auto', ranp }).then(res => {
        ...
        fs.writeFileSync(ranp, buffer, (e) => {
          if (e) return m.reply('Falló, se produjo un error, inténtelo de nuevo más tarde.')
        })
      ...
      m.reply(`Envía una imagen/video (<11 segundos) / gif con título *${usedPrefix}s* o responder uno de los anteriores con el comando`)
    }
  } catch (e) {
    console.log(e)
  }
